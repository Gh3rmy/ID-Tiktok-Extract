...
function parseUserIdFromHtml(html, username) {
  ...
  // Heurística: buscar id numérico cerca del uniqueId
...
async function fetchUserDetail(username) {
  ...
  for (const url of endpoints) {
    try {
      console.log(`[userid][api] trying ${url}`);
      ...
      if (resp.status >= 400) {
        console.warn(`[userid][api] status ${resp.status} from ${url}`);
        continue;
      }
      ...
      if (userId || secUid) {
        console.log(`[userid][api] success -> userId=${userId || '-'} secUid=${secUid || '-'}`);
        return { userId: userId ? `${userId}` : null, secUid };
      } else {
        console.log(`[userid][api] no id in response from ${url}`);
      }
    } catch (e) {
      console.warn(`[userid][api] error from ${url}: ${e?.message || e}`);
    }
  }
...
async function fetchUserFromNodeShare(username) {
  ...
  for (const url of endpoints) {
    try {
      console.log(`[userid][node] trying ${url}`);
      const resp = await axios.get(url, {
        headers,
        timeout: 15000,
        validateStatus: () => true,
      ...
      if (resp.status >= 400) {
        console.warn(`[userid][node] status ${resp.status} from ${url}`);
        continue;
      }
      ...
      if (userId || secUid) {
        console.log(`[userid][node] success -> userId=${userId || '-'} secUid=${secUid || '-'}`);
        return { userId: userId ? `${userId}` : null, secUid };
      } else {
        console.log(`[userid][node] no id in response from ${url}`);
      }
    } catch (e) {
      console.warn(`[userid][node] error from ${url}: ${e?.message || e}`);
    }
  }
  return { userId: null, secUid: null };
}
...
app.get("/api/userid", async (req, res) => {
  ...
  try {
      ...
      for (const url of candidates) {
        try {
          ...
          if (parsed?.userId || parsed?.secUid) {
            ...
            console.log(`[userid] success from ${url} -> userId=${finalUserId || '-'} secUid=${finalSecUid || '-'}`);
            break;
          } else {
            console.log(`[userid] no match from ${url}`);
          }
        } catch (e) {
          console.warn(`[userid] error from ${url}: ${e?.message || e}`);
        }
      }
    }
    ...
    console.error(`[userid] fatal username=${username} -> ${message}`);
